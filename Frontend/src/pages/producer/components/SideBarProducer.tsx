...
export default function SideBarProducer() {
  ...
  return (
    <div className={`flex flex-col container transition-all duration-300 bg-card text-card-foreground border-input ${isOpen ? 'w-64' : 'w-20'} border-r`}>
      <div className="p-4">
        <Button 
          variant="ghost" 
            ...
            <li key={index} className="px-4 py-2">
              <Link to={item.path}>
                <Button variant="ghost" className={`w-full justify-${isOpen ? 'start' : 'center'}`}>
                  ...
                  {isOpen && item.badge && (
                    <span className="ml-auto bg-primary text-white text-xs font-medium px-2 py-0.5 rounded">
                      {item.badge}
                    </span>
                  )}
                </Button>
